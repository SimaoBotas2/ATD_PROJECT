...
O script principal da Meta 2 é o \texttt{main2\_main.m}, localizado em \texttt{src/meta2/}. Foram
também desenvolvidos scripts auxiliares de teste e otimização em \texttt{src/meta2/tests/}, que
permitiram identificar os melhores parâmetros STFT e as melhores combinações de características
...
desempenho. A seleção foi apoiada pelo script \texttt{test\_all\_feature\_combinations.m}, que
avaliou exaustivamente combinações de 3 a 7 características (incluindo as da Meta 1 e da Meta 2)
com 5-fold cross-validation e um classificador de Distância Mínima.
...
% Gerada pela função confusionchart no bloco %% Fazer previsões do main2_main.m
% Nome sugerido: knn_confusion_chart.png
% -----------------------------------------------------------------------
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{knn_confusion_chart.png}
    \caption{Matriz de confusão do classificador KNN ($k=4$).}
    \label{fig:confusion_chart}
\end{figure}

% -----------------------------------------------------------------------
% FIGURA 9: Matriz de confusão em imagesc (com valores numéricos)
% Gerada pelo bloco %% 20 do main2_main.m (figura com imagesc e text)
% Nome sugerido: knn_confusion_matrix.png
% -----------------------------------------------------------------------
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{knn_confusion_matrix.png}
    \caption{Matriz de confusão detalhada do classificador KNN -- dígito real vs.\ dígito predito.}
...
% -----------------------------------------------------------------------
% NOTA: Substituir os valores de accuracy e os comentários sobre dígitos específicos
% pelos valores reais obtidos ao correr o main2_main.m.
% O script imprime no terminal: "Accuracy: XX.XX%"
% e também a acurácia por dígito.
% -----------------------------------------------------------------------

A taxa de acerto global (accuracy) obtida pelo classificador KNN foi:

\[
\text{Accuracy} = \frac{\text{Dígitos identificados corretamente}}{\text{Total de dígitos}} \times 100
\]

Analisando a matriz de confusão, é possível identificar quais os pares de dígitos mais
frequentemente confundidos pelo classificador. Em geral, dígitos foneticamente semelhantes
(como ``four'' e ``for'', ou ``nine'' e ``five'') tendem a ser confundidos com maior frequência,
dado que as suas características espectrais e temporais são mais próximas.

Os dígitos com maior taxa de acerto individual são, tipicamente, os que possuem padrões espectrais
mais distintos ou que concentram a sua energia numa faixa de frequências menos partilhada com os
restantes. Por outro lado, os dígitos com menor acurácia são aqueles cujas características
extraídas apresentam maior sobreposição com outras classes no espaço das \textit{features}.
